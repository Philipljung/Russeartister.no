...
export default function SampleCheckoutModal({ sample, onClose }: Props) {
  ...
  const fetchClientSecret = useCallback(async () => {
    try {
      const res = await fetch("/api/stripe/checkout-sample", {
        method: "POST",
          ...
          {isMobile ? (
            <div className="flex items-center gap-3 pr-10">
              <div className="shrink-0 rounded-xl overflow-hidden" style={{ width: 56, height: 56, backgroundColor: coverBg }}>
                {coverImg ? (
                  <Image src={coverImg} alt={sample.title} width={56} height={56} style={{ objectFit: "cover", width: 56, height: 56 }} />
                ) : (
              ...
              <div className="min-w-0 flex-1">
                <h2 className="truncate text-base font-bold tracking-tight" style={{ color: "#f5f5f7" }}>{sample.title}</h2>
                <p className="text-xs mt-0.5" style={{ color: "#86868b" }}>
                  {sample.producer?.display_name ?? "Ukjent"} &middot; {sample.category}
                </p>
                <p className="text-sm font-bold mt-1" style={{ color: "#f5f5f7" }}>kr {sample.price.toLocaleString("nb-NO")}</p>
              </div>
            </div>
          ) : (
            <>
              ...
              <div className="relative mb-5 w-full overflow-hidden rounded-xl" style={{ aspectRatio: "1 / 1", backgroundColor: coverBg }}>
                {coverImg ? (
                  <Image src={coverImg} alt={sample.title} fill style={{ objectFit: "cover" }} />
                ) : (
                  <div className="flex h-full w-full items-center justify-center">
              ...
              <Link
                ...
                style={{ color: "#86868b" }}
              >
                {sample.producer?.display_name ?? "Ukjent produsent"}
              </Link>

              {/* Metadata grid */}
              <div className="mb-5 grid grid-cols-2 gap-3 rounded-xl p-4" style={{ background: "rgba(255,255,255,0.04)", border: "1px solid #1e1e1e" }}>
                <div>
                  <p className="mb-0.5 text-xs" style={{ color: "#3a3a3a" }}>Type</p>
                  <p className="text-sm font-medium capitalize" style={{ color: "#f5f5f7" }}>{sample.item_type}</p>
                </div>
                <div>
                  <p className="mb-0.5 text-xs" style={{ color: "#3a3a3a" }}>Kategori</p>
                  <p className="text-sm font-medium" style={{ color: "#f5f5f7" }}>{sample.category}</p>
                </div>
                {sample.bpm && (
                  <div>
                    <p className="mb-0.5 text-xs" style={{ color: "#3a3a3a" }}>BPM</p>
                    <p className="text-sm font-medium" style={{ color: "#f5f5f7" }}>{sample.bpm}</p>
                  </div>
                )}
                {sample.vst && (
                  <div>
                    <p className="mb-0.5 text-xs" style={{ color: "#3a3a3a" }}>VST</p>
                    <p className="text-sm font-medium" style={{ color: "#f5f5f7" }}>{sample.vst}</p>
                  </div>
                )}
              </div>
              ...
              <div className="mt-auto">
                ...
                <p className="text-3xl font-bold" style={{ color: "#f5f5f7" }}>kr {sample.price.toLocaleString("nb-NO")}</p>
              </div>
            </>
          )}
        </div>
